...
from utils.poincare_utils import full_poincare_exp_map_c
from utils.spherical_utils import full_sphere_exp_map_c
...
def riemannian_grad_c(p, d_p, c, model_name):
    if model_name == "Poincare":
        p_sqnorm_c = torch.clamp(
            c * torch.sum(p.data**2, dim=-1, keepdim=True), 0, 1 - 1e-5
        )
        d_p = d_p * ((1 - p_sqnorm_c) ** 2 / 4).expand_as(d_p)
    elif model_name == "Hypersphere":
        ...
        for group in self.param_groups:
            for i, p in enumerate(group["params"]):
                ...
                if self.param_names[i] in [
                    "initial_Eh.weight",
                    "rvh.weight",
                    "initial_Eh_static.weight",
                    "initial_Eh_tail.weight",
                    "initial_Eh_tail_static.weight",
                ]:
