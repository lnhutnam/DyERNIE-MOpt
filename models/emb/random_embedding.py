...
class RandomEmbedding(torch.nn.Embedding):
    ...
    def __init__(self, num_embeddings, embedding_dim, avg_embedding_norm=1):
        ...
        curr_avg_norm = (
            d * torch.sum(rand_weight[: b - 1, :].norm(dim=1))
            + (n - (b - 1) * d) * rand_weight[b - 1, :].norm()
        ) / n
    ...
    def forward(self, input):
        ...
        if (
            torch.sum(input >= self.num_embeddings).item() != 0
            or torch.sum(input < 0).item() != 0
        ):
            raise ValueError(
                "Entries of input tensor must all be non-negative "
