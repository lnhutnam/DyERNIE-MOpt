...
from models.nn import DyERNIE_E, DyERNIE_P, DyERNIE_S
from models.optims import *
from utils.datasets import Data
from utils.general import increment_path, set_logger, str2bool
from utils.nsm_utils import getBatch_filter_all_dyn, getBatch_filter_all_static
from utils.torch_utils import to_device, save
...
class Experiment:
    ...
    def get_ept_vocab(self, data, idxs):
        ...
        for quadruple in data:
            er_vocab[
                (quadruple[idxs[0]], quadruple[idxs[1]], quadruple[idxs[3]])
            ].append(int(quadruple[idxs[2]]))
        return er_vocab

    def get_ep_vocab(self, data, idxs):
        er_vocab = defaultdict(list)
        for quadruple in data:
            er_vocab[(quadruple[idxs[0]], quadruple[idxs[1]])].append(
                int(quadruple[idxs[2]])
            )
        return er_vocab
    ...
    def batch_evaluate(self, model, d, eval_data):
        ...
        for j in range(0, len(eval_data_idxs), self.batch_size_eval):
            ...
            r_idx = to_device(
                torch.tensor(
                    np.tile(np.array([data_batch[:, 1]]).T, (1, len(d.entities)))
                )
            )
            t = to_device(
                torch.tensor(
                    np.tile(np.array([data_batch[:, 3]]).T, (1, len(d.entities))),
                    dtype=torch.double,
                )
                / self.time_rescale
            )
            e1_idx = to_device(
                torch.tensor(
                    np.tile(np.array([data_batch[:, 0]]).T, (1, len(d.entities)))
                )
            )
            e2_idx_gt = to_device(torch.tensor(np.array(data_batch[:, 2])))
            e2_idx_cand = to_device(
                torch.tensor(
                    np.tile(np.array([range(len(d.entities))]), (e1_idx.shape[0], 1))
                )
            )  # batch_size_eval * num_entities
            ...
            if not j % self.batch_size_eval:
                ...
                logging.info("Evaluated sample number: " + str(j))

        for i in range(len(model)):
            curvature = model[i].curvature.data
            # print("current curvature of model " + str(i) + ": " + str(curvature) + "\n")
            logging.info("current curvature of model " + str(i) + ": " + str(curvature) + "\n")
        ...
        mrr_raw_ob = np.mean(1.0 / np.array(ranks_raw_ob))
        ...
        logging.info("Hits @10 raw_ob: {0}".format(hits10_raw_ob))
        logging.info("Hits @3 raw_ob: {0}".format(hits3_raw_ob))
        logging.info("Hits @1 raw_ob: {0}".format(hits1_raw_ob))
        logging.info("Mean rank raw_ob: {0}".format(mr_raw_ob))
        logging.info("Mean reciprocal rank raw_ob: {0}".format(mrr_raw_ob))
        ...
        mrr_tid_ob = np.mean(1.0 / np.array(ranks_tid_ob))
        ...
        logging.info("Hits @10 tid_ob: {0}".format(hits10_tid_ob))
        logging.info("Hits @3 tid_ob: {0}".format(hits3_tid_ob))
        logging.info("Hits @1 tid_ob: {0}".format(hits1_tid_ob))
        logging.info("Mean rank tid_ob: {0}".format(mr_tid_ob))
        logging.info("Mean reciprocal rank tid_ob: {0}".format(mrr_tid_ob))
    ...
    def train_and_eval(self):
            ...
            if curvature == 0:
                model.append(
                    DyERNIE_E(
                        ...
                        use_cosh=self.use_cosh,
                        dropout=self.dropout,
                    )
                )
            elif curvature > 0:
                model.append(
                    DyERNIE_S(d, self.dim[i], self.learning_rate[i], fixed_c=curvature)
                )
            else:
                # model.append(
                #     DyERNIE_P(
        ...
        for i in range(len(model)):
            ...
            opt = RiemannianSGD(
                component.parameters(), param_names=param_names
            )  # lr=self.learning_rate,
            opts.append(opt)

        if self.resume_file:
            # print("Loading Checkpoint from {}...".format(self.resume_file))
            logging.info("Loading Checkpoint from {}...".format(self.resume_file))
            if torch.cuda.is_available():
                checkpoint = torch.load(self.resume_file)
            else:
                checkpoint = torch.load(
                    self.resume_file, map_location=torch.device("cpu")
                )
        ...
        if not os.path.isfile(file_training_path):
            ...
            file_training.write(
                "Number of training data points: %d \n" % len(train_data_idxs)
            )
        ...
        for it in range(it_start, self.num_iterations + 1):
            ...
            if not args.only_eval:
                ...
                for j in range(0, len(train_data_idxs), self.batch_size):
                    ...
                    if self.tid_nneg:
                        negsamples = getBatch_filter_all_static(
                            train_data_idxs[j : j + self.batch_size],
                            len(d.entities),
                            sr_vocab_train,
                            or_vocab=None,
                            corrupt_head=False,
                        ...
                        negsamples = getBatch_filter_all_dyn(
                            train_data_idxs[j : j + self.batch_size],
                            len(d.entities),
                            srt_vocab_train,
                            ort_vocab=None,
                    ...
                    e1_idx = torch.tensor(
                        np.concatenate(
                            (np.array([data_batch[:, 0]]).T, negsamples[:, :, 0]),
                            axis=1,
                        )
                    )
                    r_idx = torch.tensor(
                        np.concatenate(
                            (np.array([data_batch[:, 1]]).T, negsamples[:, :, 1]),
                            axis=1,
                        )
                    )
                    e2_idx = torch.tensor(
                        np.concatenate(
                            (np.array([data_batch[:, 2]]).T, negsamples[:, :, 2]),
                            axis=1,
                        )
                    )
                    t = torch.tensor(
                        np.concatenate(
                            (np.array([data_batch[:, 3]]).T, negsamples[:, :, 3]),
                            axis=1,
                        ),
                        dtype=torch.double,
                    ) / (self.time_rescale)
                    ...
                    for i in range(len(opts)):
                        ...
                        opt.step(
                            ...
                            model[i].name,
                            self.lr_cur,
                            lr=model[i].learning_rate,
                        )

                file_training = open(file_training_path, "a")
                # print("epoch: " + str(it))
                # print(
                #     "consuming time of current epoch: " + str(time.time() - start_train)
                # )
                # print("loss: " + str(np.mean(losses)))
                logging.info("epoch: " + str(it))
                logging.info(
                    "consuming time of current epoch: " + str(time.time() - start_train)
                )
                logging.info("loss: " + str(np.mean(losses)))
                file_training.write("epoch: " + str(it) + "\n")
                file_training.write(
                    "consuming time of this iter: "
                    + str(time.time() - start_train)
                    + "\n"
                )
                file_training.write("loss: " + str(np.mean(losses)) + "\n")

                save(
                    os.path.join(main_dirName, "checkpoint_latest.pt"),
                    file_training,
                    model,
                    args,
                    opts,
            ...
            with torch.no_grad():
                eval_data = (
                    d.valid_data if args.eval_data == "valid_data" else d.test_data
                )
                if not it % args.eval_ep:
                    ...
                    (
                        ...
                        hits3_raw,
                        hits10_raw,
                    ) = self.batch_evaluate(model, d, eval_data)
                    ...
                    file_training.write(
                        "mrr(time-independent filtering): " + str(mrr_tid) + "\n"
                    )
                    file_training.write(
                        "hits1(time-independent filtering): " + str(hits1_tid) + "\n"
                    )
                    file_training.write(
                        "hits3(time-independent filtering): " + str(hits3_tid) + "\n"
                    )
                    file_training.write(
                        "hits10(time-independent filtering): " + str(hits10_tid) + "\n"
                    )

                    for m, component in enumerate(model):
                        curvature = component.curvature.data
                        file_training.write(
                            "current curvature of component "
                            + str(m)
                            + ": "
                            + str(curvature)
                            + "\n"
                    ...
                    if mrr_tid > best_mrr:
                        ...
                        best_it = it
                        best_curvature = [
                            model[i].curvature.data for i in range(len(model))
                        ]

                    logging.info("best mrr tid: " + str(best_mrr))
                    logging.info("best hits1 tid: " + str(best_hits1))
                    logging.info("best hits3 tid: " + str(best_hits3))
                    logging.info("best hits10 tid: " + str(best_hits10))
                    logging.info("best epoch: " + str(best_it))
                    logging.info("best curvature: " + str(best_curvature) + "\n")
                    ...
                    save(
                        os.path.join(main_dirName, "checkpoint_{:05d}.pt".format(it)),
                        file_training,
                        model,
                        args,
                        opts,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--only_eval", type=str2bool, default=False, help="whethre only evaluation"
    )
    parser.add_argument(
        "--tid_nneg",
        type=str2bool,
    ...
    parser.add_argument(
        "--batch_size", type=int, default=512, nargs="?", help="Batch size."
    )
    parser.add_argument(
        "--batch_size_eva",
        type=int,
    ...
    parser.add_argument(
        "--nneg", type=int, default=500, nargs="?", help="Number of negative samples."
    )
    parser.add_argument(
        "--time_rescale",
        type=float,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="evaluation on the dataset per eval epochs.",
    )
    parser.add_argument(
        "--curvatures_fixed", nargs="+", type=float, default=[-0.346, -0.137, -0.855]
    )
    parser.add_argument(
        "--lr",
        nargs="+",
    ...
    parser.add_argument("--vmax", type=float, default=1.0, help="maximum velocity")
    parser.add_argument(
        "--resume_file", type=str, default="", help="model file to resume"
    )
    parser.add_argument(
        "--eval_data",
        type=str,
        default="valid_data",
        help="set eval data to be test data or valid data",
    )
    parser.add_argument(
        "--use_cosh",
        type=str2bool,
        default=False,
        help="whether use cosh in score function",
    )
    parser.add_argument(
        "--lr_cur",
        type=float,
        default=0.01,
        nargs="?",
        help="Learning rate for curvature.",
    )
    parser.add_argument(
        "--dropout", type=float, default=0, help="non_zero part of time velocity vector"
    )
    ...
    experiment = Experiment(
        args.curvatures_fixed,
        learning_rate=args.lr,
        batch_size=args.batch_size,
        num_iterations=args.num_iterations,
        dim=args.dim,
        nneg=args.nneg,
        batch_size_eval=args.batch_size_eva,
        lr_cur=args.lr_cur,
        use_cosh=args.use_cosh,
        time_rescale=args.time_rescale,
        dropout=args.dropout,
        tid_nneg=args.tid_nneg,
        vmax=args.vmax,
